...
def layout_stars(stars_to_layout):
    ...
    shuffle(stars_to_layout)
    for index, star in enumerate(stars_to_layout):
        inplusone = index + 1
        new_x_pos = inplusone * gap_size
        star.x = new_x_pos

def anamate_stars(stars_to_anamate):
    for star in stars_to_anamate:
...
def stop_animations(anamations_to_stop):
    for animation in anamations_to_stop:
